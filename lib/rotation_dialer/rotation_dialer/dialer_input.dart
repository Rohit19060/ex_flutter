...
import 'dialer_background.dart';
import 'dialer_foreground.dart';
...
class RotaryDialInput extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(ObjectFlagProperty<AsyncCallback>.has(
        'onValidatePasscode', onValidatePasscode));
    properties.add(ObjectFlagProperty<ValueSetter<int>>.has(
        'onDigitSelected', onDigitSelected));
    properties.add(DiagnosticsProperty<bool>(
        'passcodeAnimationInProgress', passcodeAnimationInProgress));
    properties.add(DoubleProperty('pagePadding', pagePadding));
...
class _RotaryDialInputState extends State<RotaryDialInput>
  ...
  void _onPanUpdate(DragUpdateDetails details, Offset centerOffset) {
    ...
    if (currentDirection * previousDirection < 0.0) {
      return;
    }

    final angle = _startAngleOffset + currentDirection - previousDirection;

    if (angle < 0.0 || angle >= RotaryDialConstants.maxRotaryRingAngle) {
      return;
    }
  ...
  void _addDigit(int index) {
    ...
    _rotateDialToStart().then((_) async => widget.onValidatePasscode());
  }
            ...
            for (var i = 0; i < inputValues.length; i++)
              Transform.translate(
                offset: Offset.fromDirection(
                    (i + 1) * -math.pi / 6, dialNumberDistanceFromCenter),
